...
import collections
# Use torchax version of model_management for TPU support
from comfy import model_management_torchax as model_management
import math
import logging
...
try:
    ...
    JAX_AVAILABLE = False
    mark_sharding = lambda x, spec: x  # No-op fallback
...
def finalize_default_conds(model: 'BaseModel', hooked_to_run: dict[comfy.hooks.HookGroup,list[tuple[tuple,int]]], default_conds: list[list[dict]], x_in, timestep, model_options):
    ...
    for i, mult in enumerate(default_mults):
        ...
        if len(default_conds[i]) == 0:
            continue
        # Avoid inplace operation for TPU/XLA compatibility
