"""
ComfyUI Flux Model (Torchax TPU Version)

This module provides the TPU-optimized Flux transformer model for ComfyUI.
Original code from: https://github.com/black-forest-labs/flux

Key TPU optimizations:
1. Sharding constraints on model inputs/outputs
2. JAX-compatible forward pass
3. Optimized attention using Splash Attention
4. Weight sharding across TPU cores
"""
...
# 使用 torchax 版本的 layers
from .layers_torchax import (
    DoubleStreamBlock,
    EmbedND,
    LastLayer,
    MLPEmbedder,
...
# ============================================================================
# Flux Model (Torchax TPU Version)
# ============================================================================
class Flux(nn.Module):
    """
    Transformer model for flow matching on sequences (Torchax TPU Version).
    
    This is the main Flux DiT model optimized for TPU execution using torchax.
    Key optimizations:
    - Sharding constraints on input tensors
    ...
    def __init__(self, image_model=None, final_layer=True, dtype=None, device=None, operations=None, **kwargs):
        ...
        # Double Stream Blocks (使用 torchax 版本)
        ...
        # Single Stream Blocks (使用 torchax 版本)
    ...
    def forward_orig(
        ...
        """
        Original forward implementation (Torchax TPU Version).
        
        TPU optimization: Apply sharding constraints at model entry.
        """
        # TPU: Apply sharding constraint at model entry
        ...
        for i, block in enumerate(self.double_blocks):
            ...
            if control is not None: # Controlnet
                ...
                if i < len(control_i):
                    add = control_i[i]
                    if add is not None:
                        img = img.clone()  # 避免 inplace 操作
                        img[:, :add.shape[1]] = img[:, :add.shape[1]] + add
            ...
            if control is not None: # Controlnet
                ...
                if i < len(control_o):
                    ...
                    if add is not None:
                        # 避免 inplace 操作
                        img = img.clone()
                        img[:, txt.shape[1] : txt.shape[1] + add.shape[1], ...] = \
        ...
        # TPU: Apply sharding constraint at model exit
        if TORCHAX_AVAILABLE:
            img = mark_sharding(img, P())
    ...
    def process_img(self, x, index=0, h_offset=0, w_offset=0, transformer_options={}):
        """Process image input into sequence format."""
        bs, c, h, w = x.shape
