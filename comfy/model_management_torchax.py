...
def _init_tpu():
    ...
    global tpu_available, tpu_device_count, tpu_mesh
    if JAX_AVAILABLE:
        try:
            devices = jax.devices('tpu')
            tpu_device_count = len(devices)
            tpu_available = tpu_device_count > 0
            if tpu_available:
                # Create mesh for tensor parallelism
                tpu_mesh = Mesh(mesh_utils.create_device_mesh((tpu_device_count,)), ('tp',))
                logging.info(f"TPU available: {tpu_device_count} devices")
        except:
            tpu_available = False
            tpu_device_count = 0
    return tpu_available

# Try to initialize TPU at module load
_init_tpu()
...
def get_tpu_mesh():
    """Get the TPU mesh for sharding."""
    global tpu_mesh
    return tpu_mesh

def get_tpu_device_count():
    """Get the number of TPU devices."""
    global tpu_device_count
    return tpu_device_count

def get_tpu_memory():
    """Get TPU HBM memory (approximate per chip)."""
    if is_tpu():
        # TPU v5e: 16GB HBM per chip, TPU v4: 32GB per chip
...
def get_torch_device():
    ...
    # TPU takes priority if available
    if is_tpu():
...
def get_total_memory(dev=None, torch_total_too=False):
    ...
    # Handle TPU memory
    if dev is not None and hasattr(dev, 'type') and dev.type == 'xla':
        mem_total = get_tpu_memory() * get_tpu_device_count()
...
def get_free_memory(dev=None, torch_free_too=False):
    ...
    # Handle TPU memory - TPU doesn't have a direct free memory API
    # Return approximate available HBM
    if dev is not None and hasattr(dev, 'type') and dev.type == 'xla':
        # TPU HBM is managed by JAX, return total memory as approximation
        mem_total = get_tpu_memory() * get_tpu_device_count()
        mem_free_total = int(mem_total * 0.9)  # Assume 90% available
...
def soft_empty_cache(force=False):
    ...
    # TPU/XLA memory is managed by JAX
    if is_tpu():
        if JAX_AVAILABLE:
            # JAX doesn't have explicit cache clearing like CUDA
